...
app.config['MAIL_USERNAME'] = 'crestinsurebroker@gmail.com'
app.config['MAIL_PASSWORD'] = 'gvwa wojk dlqc liiq'
app.config['MAIL_DEFAULT_SENDER'] = 'crestinsurebroker@gmail.com'

mail = Mail(app)
def get_db():
    conn = mysql.connector.connect(
        host="database-1.cc184ogcw4v7.us-east-1.rds.amazonaws.com",
        # host="127.0.0.1",
        # user="root",
        user="admin",
        password="Ps5638806",
...
@app.route("/posp")
def posp():
    return render_template("posp.html", time=time)
...
def capture_mobile():
    ...
    # Optional: Save mobile immediately if user record exists later
...
def signup():
    if request.method == "POST":
        ...
        secret_key = "6LecGoMsAAAAAIpVbHZUAGtCKiQUba_yk8Aklfiy"
...
def dashboard():
    ...
    if request.method == "GET":
        ...
        # dropdown lists
        managers = ["Ravi Prakash","Dilshad Ali"]
...
# ------------------ POST → SAVE submitted data ------------------
    try:
        ...
        re_account   = request.form.get("re_account")
...
def get_user_info():
    ...
    conn, cursor = get_db()
    cursor.execute("SELECT agent_code, application_no, status, doj FROM users WHERE email=%s", (user_email,))
    row = cursor.fetchone()
    conn.close()
    ...
    agent_code, application_no, status, doj = row["agent_code"], row["application_no"], row["status"], row["doj"]
        ...
        "message": {
            ...
            "body": {
                ...
                "content": (
                    ...
                    f"📧 pospsupport@crestinsure.com\n"
                    f"🌐 www.crestinsure.com\n"
                )
            },
            "toRecipients": [
                {"emailAddress": {"address": to_email}}
...
#______________Admin Process___________
...
def verify_user(user_id):
    ...
    # Update status
    cursor.execute("UPDATE users SET status='Verified' WHERE id=%s", (user_id,))
    conn.commit()


    # Get user email
    cursor.execute("SELECT email, first_name, agent_code FROM users WHERE id=%s", (user_id,))
    user = cursor.fetchone()
    ...
    code = user['agent_code']

    send_email(email, "Verification Successful",
               f"Hello {name},\n\nYour documents have been VERIFIED successfully.Your partner code is {code}\n\nThank you.")
...
@app.route("/admin/discrepancy/<int:user_id>", methods=["POST"])
def mark_discrepancy(user_id):
    note = request.form.get("note")  # Get input from the admin

    conn, cursor = get_db()
    try:
        # Only update the status column
        cursor.execute("UPDATE users SET status=%s WHERE id=%s", (note, user_id))
        conn.commit()
    ...
    # Send email with the admin's note
    if user:
        email = user['email']
        name = user['first_name']
        send_email(
            email,
            f"⚠ Discrepancy Identified {name}",
            f"Hello {name},\n\nThere is a discrepancy in your documents: \n\nIssue-{note}\nPlease review and update accordingly.\n\nThank you."
        )
...
UPLOAD_FOLDER = "static/uploads"  # Make sure this folder exists or will be created
...
def user(user_id):
    ...
    try:
        ...
        # ---- Documents ----
        doc_fields = ["profile_photo", "pan_doc", "address_proof", "education_cert", "bank_proof"]
        doc_paths = {}
        for field in doc_fields:
            ...
            if f and f.filename:
                ...
                f.save(save_path)
                doc_paths[field] = save_path.replace("\\", "/")  # Windows-safe
        ...
        """, (
            ...
            data.get("account"),
            data.get("re_account"),
            doc_paths.get("profile_photo"),
            doc_paths.get("pan_doc"),
            doc_paths.get("address_proof"),
            doc_paths.get("education_cert"),
            doc_paths.get("bank_proof"),
            user_id
        ))
        conn.commit()
    except mysql.connector.errors.DataError as err:
            # Show the error in a popup window
...
def admin_reports():
    ...
    # Active Claims
    #cursor.execute("SELECT COUNT(*) AS total FROM claims WHERE status='active'")
    #active_claims = cursor.fetchone()["total"]
...
def download_users():
    ...
    writer.writerow(["ID","Email","Mobile","PAN","Created"])
...
def contact():
    ...
    if request.method == "POST":
        ...
        if not name or not email or not phone or not message:
            ...
            return redirect(url_for("contact-us.html"))

        try:
            msg = Message(
                subject="New Contact Enquiry - Crest Insure Broker",
                recipients=["crestinsurebroker@gmail.com"]
            )
...
"""
            ...
            flash("Your message has been sent successfully. You will hear from us soon! ✅", "success")

        except Exception as e:
            print(e)
            flash("Mail not sent ❌ SMTP is blocked or wrong password", "danger")
